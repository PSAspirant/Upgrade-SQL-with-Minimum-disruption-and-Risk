...
Invoke-Sqlcmd -ServerInstance SQL01 -Database master -InputFile "G:\SQLServer2019\Monitoring Replication using Scripts.sql" |`
...
We will connect to SQL03 AG Replica and Upgrade to SQL2019 # First Level of Upgrade
#>

### This dynamic management view exposes state information on both the primary and secondary replicas
"We will connect to SQL03 AG Replica and Upgrade to SQL2019 # First Level of Upgrade" | Out-File G:\SQLServer2019\Log.txt -Append
...
Invoke-Sqlcmd -ServerInstance SQL01 -Database master -Query $setaync | Out-File G:\SQLServer2019\Log.txt -Append
$ser = @('SQL01','SQL02','SQL03')
foreach($se in $ser){
...
Start-Sleep 45
 ...
 #Starting the InPlace Upgrade from SQL Server 2017 --> SQL Server 2019 in SQL03 AG Replica

 Invoke-Command -ComputerName SQL03 -ScriptBlock {
...
"$time : $user" | Out-File G:\SQLServer2019\Log.txt -Append
 ...
 "$time : Starting the In-Place Upgrade of SQL Server 2017 --> SQL Server 2019 in SQL03" |`
  Out-File G:\SQLServer2019\Log.txt -Append
...
Restart-Computer -ComputerName SQL03 -Force
Start-Sleep 30
...
Start-Sleep 15
 ...
 ## Setting SQL02 to Asynchronous Mode ## Run the below command in Primary Replica of AG ###
 ### so that transactions on the primary can be committed without waiting for confirmation from the secondary replica ####
 "We will connect to SQL02 AG Replica and Upgrade to SQL2019 # Second Level of Upgrade" | Out-File G:\SQLServer2019\Log.txt -Append
...
<#
Now the SQL Server 2019 install media is copied to all AG Replicas . 
We will connect to SQL02 AG Replica and Upgrade to SQL2019 # First Level of Upgrade
#>
...
Restart-Computer -ComputerName SQL02 -Force
...
Invoke-Sqlcmd -ServerInstance SQL01 -Database master -Query $setaync

Start-Sleep 45

#### Failover AOAG from SQL01 --> SQL02

$AGFailoverSQL02 = "USE [master]
GO
 
ALTER AVAILABILITY GROUP [AOAG1] FAILOVER;
GO"

Invoke-Sqlcmd -ServerInstance SQL01 -Database master -Query $AGFailoverSQL02
...
Start-Sleep 45
 ...
 ## Setting SQL01 to Asynchronous Mode ## Run the below command in Primary Replica of AG ###
 ### so that transactions on the primary can be committed without waiting for confirmation from the secondary replica ####
 "We will connect to SQL01 AG Replica and Upgrade to SQL2019 # Second Level of Upgrade" | Out-File G:\SQLServer2019\Log.txt -Append
...
Invoke-Sqlcmd -ServerInstance SQL02 -Database master -Query $setaync | Out-File G:\SQLServer2019\Log.txt -Append
...
Invoke-Command -ComputerName SQL01 -ScriptBlock {
...
Restart-Computer -ComputerName SQL01 -Force


## Setting SQL01 to Synchronous Mode ## Run the below command in Primary Replica of AG ###
...
Invoke-Sqlcmd -ServerInstance SQL02 -Database master -Query $setaync

Start-Sleep 45

#### Failover AOAG from SQL02 --> SQL01

$AGFailoverSQL01 = "USE [master]
GO
 
ALTER AVAILABILITY GROUP [AOAG1] FAILOVER;
GO"

Invoke-Sqlcmd -ServerInstance SQL02 -Database master -Query $AGFailoverSQL01
...
#### Failback AOAG from SQL02 --> SQL01
...
Invoke-Sqlcmd -ServerInstance SQL01 -Database master -Query $AGState | Out-File G:\SQLServer2019\Log.txt -Append
